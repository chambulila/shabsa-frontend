...
import { CircleCheckBigIcon, Layout } from 'lucide-react'
import { useParams } from 'next/navigation'
import React from 'react'

function ServiceDescription() {
    const { serviceId } = useParams(); // Get service id from URL parameters } = useParams(); // Get service id from URL parameters
                ...
                <CardFooter className="flex flex-col items-center gap-3">
                    {service?.subServices?.map(sub => (
                        <div key={sub?.name} className="flex  gap-1 items-center ">
                            <div><CircleCheckBigIcon className="text-yellow-600" /></div>
