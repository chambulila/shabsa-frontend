...
export default function CreateBrand({ brandToEdit = null, setCreateBrand, handleClose, getBrands }) {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            let response;
            if (brandToEdit) {
                response = await brandAndCategoryService.updateBrand(brandToEdit.uuid, formData);
            } else {
                response = await brandAndCategoryService.createBrand(formData);
            }
    ...
    return (
        <form className="space-y-4">
            ...
            <div className="flex gap-3">
                ...
                <Button disabled={creatingBrand} type="submit" onClick={handleSubmit} >Submit</Button>
            </div>
        </form>
    )
}
